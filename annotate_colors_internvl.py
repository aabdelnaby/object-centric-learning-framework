"""Annotate part-crop colors with InternVL3-14B, forced to one of 11 basic colors.

Reads ``parts_color_vqa_masks.csv`` + the pre-extracted crops in ``--crops-dir``
(see ``extract_part_crops.py``), shows each part-only crop to InternVL3-14B, and
asks for the single dominant color, constrained to the Berlin-Kay basic set:

    black white gray red orange yellow green blue purple pink brown

The raw response is parsed and synonym-mapped down to that set (``unknown`` if it
can't be resolved). Sharded for a SLURM job array: shard K processes every row with
``question_index % num_shards == K`` and writes ``preds/predictions_shard_K.csv``.
Resumable via ``--skip-done`` (skips question_indices already in the shard CSV).

Run (single GPU):
    conda run -n internvl_env python annotate_colors_internvl.py --shard 0 --num-shards 1
"""
...
def main():
    ...
    ap.add_argument("--crops-dir", default="FG-datset/ade20k/part_crops")
    ap.add_argument("--out-dir", default="FG-datset/ade20k/preds")
