"""Build the PACO-LVIS part-colour question CSV (thesis Section 4.1).

Uses the PACO-LVIS human part annotations on COCO-2017 images to emit the question

    "What is the color of the <part> of the <object>?"

for every (object, part) pair that is referentially unambiguous:

  * the OBJECT category appears exactly once among the image's PACO object instances,
  * the PART category appears exactly once among that object's parts (linked via ``obj_ann_id``),
  * the part mask has area >= ``--min-area`` pixels (400), and
  * the part is not in the blacklist of typically repeated parts (leg, ear, wheel, side, ...).

``geom_label`` holds PACO's human dominant colour mapped to 11 basic colours (``unknown`` where
PACO annotated none; that is ~18 % of parts). The answer column ``label`` is left blank here:
``finalize_csv.py`` fills it with ``geom_label`` and, where that is unknown, with the InternVL
colour of the part crop (``extract_part_crops.py`` + ``annotate_colors_internvl.py``).

Outputs under ``--out-dir`` (default ``data/paco``):
    paco_<split>.csv                        14-column question CSV (label / raw_response blank)
    paco_<split>_masks.csv                  question_index, object/part names, image_path, part_mask_path
    part_masks/<split>/<question_index>.png binary part mask (0/255), used for grounding evaluation
Image paths are relative to ``--coco-root``, mask paths relative to ``--out-dir``.

    python data_prep/paco/build_vqa_csv.py --split train
    python data_prep/paco/build_vqa_csv.py --split val
"""
...
def main():
    ...
    ap.add_argument("--out-dir", default="data/paco")
    ...
    for img_id in img_ids:
        ...
        img_path = im["file_name"]                       # relative to --coco-root (e.g. train2017/xxx.jpg)
        if not os.path.isfile(os.path.join(args.coco_root, img_path)):
            raise FileNotFoundError(f"COCO image missing: {os.path.join(args.coco_root, img_path)}")
        ...
        for oaid, plist in parts_by_parent.items():
            ...
            for pp in plist:
                ...
                mask_path = os.path.join("part_masks", args.split, f"{qi}.png")   # relative to --out-dir
                Image.fromarray((m * 255).astype(np.uint8)).save(os.path.join(args.out_dir, mask_path))
    ...
    out_vqa = os.path.join(args.out_dir, f"paco_{args.split}.csv")
    out_masks = os.path.join(args.out_dir, f"paco_{args.split}_masks.csv")
